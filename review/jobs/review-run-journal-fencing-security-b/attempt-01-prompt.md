...
Job: `review-run-journal-fencing-security-b` (surface-review)
...
## Assignment: deep independent review of `run-journal-fencing` — Run-control SQLite journal and fencing
...
Primary paths: internal/runcontrol/sqlite.go; internal/runcontrol/lifecycle.go; internal/runcontrol/model.go; internal/runcontrol/process_linux.go; internal/runcontrol/id.go
State: .ultraplan/run-control.db (runs/attempts/events/operation_aliases/reconciliation_log); .ultraplan/run-control.log
Trust boundaries: durable journal rows re-read as input; agent-derived event payloads admitted only after sanitizer allowlist

Review lens: **Independent adversarial security review. Require a concrete attacker-controlled source, path, sink/consequence, and missing control before calling something exploitable.**

Read the neutral context pack at `/home/antonioborgerees/coding/ultraplan/ultraplan-quality-gauntlet/review/jobs/context-run-journal-fencing/result.md`. Then inspect the actual source and tests yourself. Do not assume the context pack is complete.
